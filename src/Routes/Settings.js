...
function Settings({ setLogin }) {
  ...
  const getUser = () => {
    axios.get('https://lovechat-api.onrender.com/myprofile', {
      headers: {
  ...
  const handleFormSubmit = (e) => {
    e.preventDefault();
    axios.put('https://lovechat-api.onrender.com/update-profile', updateUser, {
      headers: {
        'Authorization': localStorage.getItem('jwt'),
      },
    }).then((response) => {
  ...
  let deleteHandler = (e)=>{
    e.preventDefault();
    axios.delete('https://lovechat-api.onrender.com/delete-account', {
      headers: {
        'Authorization': localStorage.getItem('jwt'),
      },
    }).then((response) => {
    ...
    <center>
      <div className="card mb-4 mt-4 setting-section" style={{ width: "50%" , borderRadius : '12px'}}>
        <div className='profile-image'>
          <img src={user.avatar} className="card-img-top" alt="..." style={{ borderRadius: '50%', border: '4px inset tomato' }} />
          ...
          <div className="modal-content">
            ...
            <center className="modal-body">
              <form className='post-form' onSubmit={handleFormSubmit}>
                <input type="text" name="username" placeholder="User name" className='m-2' onChange={updateHandler} style={{ width: '70%' }}/>
                <input type='text' name="bio" placeholder="bio" className='m-2' onChange={updateHandler} style={{ width: '70%', height: '50px' }} />
                <input type="text" name="email" placeholder="email" className='m-2' onChange={updateHandler} style={{ width: '70%' }} />
                <input type="text" name="location" placeholder="Location" className='m-2' onChange={updateHandler} style={{ width: '70%' }} />
                <div className="modal-footer">
